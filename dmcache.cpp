/*
Simulation of a cache that uses direct mapping.
...
The direct mapped cache produces as output a file named dm-out.txt. Each line
...
#define ADDRESS_LINE_BITS 6 // number of address bits used to designate cache line
#define MASK_6_BITS 63
...
{
public:
	Address (int tag, int line) {
		this->tag = tag;
		this->line = line;
		this->offset = 0;
		this->value = (tag << (ADDRESS_OFFSET_BITS + ADDRESS_LINE_BITS)) | (line << ADDRESS_OFFSET_BITS);
	}
	Address (unsigned short value) {
		this->value = value;
		tag = (value >> ADDRESS_OFFSET_BITS + ADDRESS_LINE_BITS);
		line = (value >> ADDRESS_OFFSET_BITS) & MASK_6_BITS;
...
namespace CacheOps
{
	...
	CacheLine * getLine(Address &address, bool isRead)
	{
		...
		#ifdef DEBUG
		if (isRead)
			cout << "hit comparison " << (int) p_line->tag << ' ' << (int) address.getTag() << endl;
		#endif
		if (!isHit) // cache miss
		{
			if (p_line->dirty) // write-back, evict
			{
				Address writebackAddress(p_line->tag, address.getLine());
			...
			for (int i = 0; i < BLOCK_BYTES; i++)
				p_line->value[i] = RAM[address.ramBlock() + i]; // read from RAM into cache line
		}
		if (isRead)
			for (int i = BLOCK_BYTES-1; i >= 0; i--)
		...
		if (isRead)
		{
			...
			#ifdef DEBUG
			output << ' ' << " - line " << (int) address.getLine();
			#endif
			output << endl;
		}
		return p_line;
	...
	if (input)
	{
		...
		if (output)
		{
			while (!input.eof())
			{
				...
				#ifdef DEBUG
				cout << "address " << hex << uppercase << addressInt
					<< " " << setfill('0') << setw(2) << opcode << " " << setw(2) << value
					<< "\t"
					<< " tag " << (int) address.getTag() 
					<< " line " << (int) address.getLine()
